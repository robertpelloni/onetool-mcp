...
def _patch_env(env: dict):
    ...
    def patcher():
        stack = ExitStack()
        stack.enter_context(
            patch("ottools.ot_forge._get_tools_config", return_value={
                "claude": {"stub_path": ".claude/skills/{name}/SKILL.md"},
                "codex": {"stub_path": ".codex/skills/{name}/SKILL.md"},
                "opencode": {"stub_path": ".opencode/skills/{name}/SKILL.md"},
            })
        ...
        stack.enter_context(
            patch("ottools.ot_forge._list_bundled_skills", return_value=["my-skill", "other-skill"])
        )
        stack.enter_context(
            patch("ottools.ot_forge._get_skill_description", side_effect=lambda n: f"Desc for {n}")
        )
        stack.enter_context(
...
def test_get_tools_config_loads_yaml() -> None:
    ...
    from ottools.ot_forge import _get_tools_config
